...
from utils.options import HiDDenConfiguration, TrainingOptions
...
def load_last_checkpoint(checkpoint_folder):
    ...
    print("Loading", last_checkpoint_file)
    checkpoint = torch.load(last_checkpoint_file)

    return checkpoint #, last_checkpoint_file

def load_best_checkpoint(checkpoint_folder: str, keys=None):
    val_df = pd.read_csv(os.path.join(checkpoint_folder, 'validation.csv'))
    if keys == None:
        if 'baseline' in checkpoint_folder:
            keys = ['bitwise-error', 'total_loss']
        else:
            keys = ['penalty_psnr', 'bitwise-error', 'total_loss'] 
    
    val_df.sort_values(keys, inplace=True)
    checkpoint_file = checkpoint_folder.removeprefix('HiDDen/runs_lanz/').split(' ')[0]
    for idx in range(val_df.index.size):
        epoch = int(val_df.iloc[idx]['epoch'])
        print(epoch)
        if os.path.isfile(os.path.join(checkpoint_folder, 'checkpoints', f'{checkpoint_file}--epoch-{epoch}.pyt')):
            checkpoint_file = f'{checkpoint_file}--epoch-{epoch}.pyt'
            break
    
    print(f'Loading {checkpoint_file}')
    return torch.load(os.path.join(checkpoint_folder, 'checkpoints', checkpoint_file))
...
def load_options(options_file_name) -> (TrainingOptions, HiDDenConfiguration, dict): # type: ignore
    ...
    with open(os.path.join(options_file_name), 'rb') as f:
        ...
        if not hasattr(train_options, 'early_stop'):
            setattr(train_options, 'early_stop', True)

    return train_options, hidden_config, noise_config


def get_data_loaders(path: str, hidden_config: HiDDenConfiguration, train_options: TrainingOptions):
    ...
    train_set = ChestXRayDataset(path, train=True, transform=transform)
    # 78484 = 70% total dataset 
    # 16818 = 15% total dataset
    train_set, val_set = torch.utils.data.random_split(train_set, [78484, 16818], generator=torch.Generator().manual_seed(42))

    train_loader = torch.utils.data.DataLoader(train_set, batch_size=train_options.batch_size, shuffle=True, num_workers=16)
    validation_loader = torch.utils.data.DataLoader(val_set, batch_size=train_options.batch_size, num_workers=12)
...
def sort_ChestXRay(imgs_dir, dest_dir=''):
    img_list = os.listdir(imgs_dir)
    if dest_dir == '':
        dest_dir = imgs_dir
    for split in ["train_val", "test"]:
        with open(os.path.join(dest_dir, f"{split}_list.txt")) as f:
            count = 0
            lines = f.readlines()
            for img in lines:
                img = img.strip("\n")
                if img in img_list:
                    count += 1
                    shutil.move(os.path.join(imgs_dir, img), os.path.join(dest_dir, split, img))
            print(f"{count} images moved to {split} subdirectory")


def extract_training_params(filename: str):
    param_dict = {}
    # no_date = filename.split(' ')
    if 'baseline' in filename.lower():
        param_dict['baseline'] = None
    elif 'coeff' in filename.lower():
        if '2025.' in filename:
            filename = filename.split(' ')[0]
        
        to_keep_idx = filename.find('coeff')
        filename = filename[to_keep_idx:]
        ...
        if param_dict['factor'] > 1:
            param_dict['fixed'] = False
        else:
